...
结果表明，像素量化模型下约 `22%～25%` 的有效观测帧存在超过 `20%` 的测距误差。距离比例误差对归一化 LOS 方向影响有限，但会影响提前时间、目标重捕获和速度追踪项。`stable` 显著降低命令换向频率和 jerk RMS，同时增加相位滞后。cosine 测量链路消融中，干净角度观测和仅像素量化的单机命中率均为 `93%`，加入 `0.5°` 角噪声后约为 `62%`，表明 LOS-rate 估计对角度噪声及滤波延迟更敏感。
...
建议每次修改控制公式时同步完成：更新 README 参数、增加或修改回归测试、运行 `python -m unittest discover -s tests -v`、至少运行一次单机和三机示例，再提交。不要把上游 MAPPO 工程的大模型权重、训练日志或生成的 `outputs/` 推到这个纯 LOS 仓库。
