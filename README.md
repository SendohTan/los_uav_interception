...
> **2026-08-18 / v0.2.0：**针对实机速度指令震荡，新增滑窗 LOS-rate 回归、LOS 角速度软阈值、期望速度低通与变化率限制、加速度低通、失帧/超时安全门和震荡诊断。完整原理、参数、测试数据和实机限制见 [`docs/STABILIZATION_2026-08-18.md`](docs/STABILIZATION_2026-08-18.md)。
...
`2026-08-18` 正式测试采用逐帧 `0.5°` 角噪声、`7.5%` 固定测距偏差和 `0.5%` 测距抖动。单机每种配置测试 400 回合，三机每种配置测试 200 回合。

配置含义：

- `legacy`：v0.1.0 两帧差分和未整形导引指令；
- `stable`：默认均衡配置，兼顾导引响应与指令平滑；
- `conservative`：增强滤波的保守配置；
- `flight_test`：最低指令变化率配置，用于低速 HIL 和受控首飞。
...
结果表明，滤波强度提高后，指令换向频率和 jerk RMS 持续下降，同时复杂轨迹导引响应变慢、命中率下降。默认 `stable` 配置用于算法仿真；`LOSGuidanceConfig.flight_test()` 用于低速、开阔场地、具备人工接管条件的首轮 HIL 和实飞验证。
